...
typedef struct matrix_st{
	...
	float m[MAXLENGTH][4];
	float max_remain_gain_forward[MAXLENGTH];//zzz
	float max_remain_gain_reverse[MAXLENGTH];//zzz
...
/***********************************************/
{
	...
	if(ch) *ch = '\0';
	len = strlen(string);
	while(len>0 && (string[len-1]==separator || string[len-1]==' '||string[len-1]=='\r')) --len;
	string[len] = '\0';
	ch = string;
	while(1){
		items[i] = ch;
...
/***********************************************/
{
	...
	check(buffer = (char*)malloc(500*sizeof(char)));
	fp = fopen(filename,"r");
	ch = truncate_filename(filename);
	if(!fp){ printf("ERROR: Input file %s not found\n",ch); exit(1); }
	for(i=0; i<2; ++i){
		fgets(buffer,499,fp);
	...
	rewind(fp);
	while(fgets(buffer,499,fp)){
		if(strstr(buffer,"DE")==buffer) N++;
	}
	rewind(fp);
	if(N > p->N_matrix){
		N = p->N_matrix;
	...
	char matlistfilename[strlen(p->outputFile)+20];
	sprintf(matlistfilename,"%s/matlist.txt\0",p->outputFile);
	printf("\n%s\n",matlistfilename);
	fflush(stdout);
	FILE* matlistfile=fopen(matlistfilename,"w");

	double log10 = log(10);
	while(fgets(buffer,499,fp)){
		int done=0;
		float sumMax=0;
		while(strstr(buffer,"DE")!=buffer && !done){
			if(!fgets(buffer,499,fp)) done=1;
		}

		if(N>=NNN || done){ break; }
		int n = split_string(&buffer[3],items,10,'\t');
		...
		while(fgets(buffer,499,fp)){
			float sum=0, max=0, x;
			if(strstr(buffer,"PO")==buffer) {continue;}
			if(strlen(buffer)<5){ break; }
			sscanf(buffer,"%d%f%f%f%f",&i,&m[0],&m[1],&m[2],&m[3]);
			//if(i!=pos){ printf("Wrong file format in line: %s",buffer); exit(1); }
			i=pos;
			for(i=0; i<4; ++i){
				sum += m[i];
			}
		...
		//scoreThresh already initiate to 2*FDR
		if(M[N].scoreThresh && p->threshAdd){
			M[N].scoreThresh += p->threshAdd;
			if(M[N].scoreThresh > sumMax){ M[N].scoreThresh = sumMax; }// otherwise will always zero match
		}
		if(M[N].scoreThresh < -999999) M[N].scoreThresh = -999999;
		fprintf(matlistfile,"%s\t%d\n",M[N].name,M[N].len);
		N++;
	}
	fclose(matlistfile);
	fclose(fp);
...
/***********************************************/
{
	...
	check(buffer = (char*)malloc(300*sizeof(char)));
	check(seqname = (char*)malloc(300*sizeof(char)));
	check(seqname1 = (char*)malloc(300*sizeof(char)));
	seq[0] = '\0';
	fp = fopen(filename,"r");
	ch = truncate_filename(filename);
	if(!fp){ printf("ERROR: Input file %s not found\n",ch); exit(1); }
...
/***********************************************/
{
	...
	check(buffer = (char*)malloc(1000*sizeof(char)));
	check(seqname = (char*)malloc(300*sizeof(char)));
	check(seqname1 = (char*)malloc(300*sizeof(char)));
	check(items = (char**)malloc(200*sizeof(char*)));
	check(cons = (int*)malloc(MAXINT*sizeof(int)));
	fp = fopen(filename,"r");
	ch = truncate_filename(filename);
	if(!fp){ printf("ERROR: Conservation file %s not found",ch); exit(1); }
	fgets(buffer,999,fp);
...
/***********************************************/
{
	float maxEntropy = 2, pp[4], entropy, *info, infoCum, sum, sumMean, sumVar, sumMax, sumMean1, sumVar1,maxP;
	float *score;
	...
	for(i=0; i<nPattern; ++i){
		...
		if(M->len > 8){
			...
			for(j=0; j<M->len; ++j)
			{
				M->max_remain_gain_forward[j]=0;
				M->max_remain_gain_reverse[j]=0;// 4 means the score of FDR 0.0001, not need to consider early stop
			}
			/*
			   ...
			   score[rep] = sum; //score for the non-core part
		//printf("%.4f\n",sum);
		}
		sortem(NSIM, score, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
		int nThr = NSIM*0.999;//the 0.001 exceptioally good score for the non-core part
		adjust = score[nThr]+0.01;
		*/
			//instead of MC, I directly find the best score for the non-core part
			sum = 0;
			...
			for(j=0; j<M->len; ++j){
				...
				for(k=0; k<4; ++k)
					if(M->m[j][k]>maxP)
						maxP=M->m[j][k];
				M->max_remain_gain_forward[j]=maxP;
				M->max_remain_gain_reverse[M->len-j-1]=sum;
				sum+=maxP;

				//we can record the best score for non-core part, enable early stop when the partial matching is bad
			}
			adjust=sum;
			...
			for(j=0; j<M->len; ++j)
			{
				sum+=M->max_remain_gain_forward[M->len-j-1];
				M->max_remain_gain_forward[M->len-j]=sum-M->max_remain_gain_forward[M->len-j-1];
			}

		}
		if(!M->scoreThresh && !p->falsePositives){ p->falsePositives=1; }
		if(p->falsePositives){
			M->scoreThresh = 1.3;//log(10000.0/ p->falsePositives/2)/log10;//using 2times FDR as starting point//1.3;
		}
		M->scoreThreshCore = M->scoreThresh - adjust;
		//the bad case is that the non-core part is very high score, then the corethresh will be very low
		////ADDED
		//M->scoreThreshCore=-999999;
		//M->scoreThresh=-999999;
...
void  find_motifs (int **sequence, long *seqlen, long nseq, LOOKUP *lookupTable,
		MATRIX *Matrix, int nPattern, PARAM *p, char* outdir, char **seqNames, int **conservation, float **scores, long *nhits,int bgmode)
/***********************************************/
{

	long i, j, k, iseq, pos, ind, lastMissing=0;
	int mult, *seq, dir, *pattern, N, x,mmm, im, *cons=NULL;
	int nstack=0;
	...
	check(fpArray=(FILE **)malloc(nPattern*sizeof(FILE *)));
	//printf("%d\n",nPattern);
	for(im=0; im<nPattern; ++im)
	{
		nhits[im] = 0;
		//if(im<1000) // the operation system limit
		//{
		//		check(filename = (char*)calloc(100,sizeof(char)));
		//		strcpy(filename,outdir);
		//		 strcat(filename,"/");
		//		 strcat(filename,Matrix[im].name);
		//		 strcat(filename,".pos");
		//		fpArray[im]=fopen(filename,"w");
		//		
		//}

	}
	char myfilename[500];
	for(mmm=0; mmm<nPattern; mmm=mmm+1000)
	{			

		for(im= mmm; im<mmm+1000&&im<nPattern; ++im)
		{
			sprintf(myfilename,"%s/%s.pos",outdir,Matrix[im].name);
			if (bgmode==0){
				fpArray[im]=fopen(myfilename,"w+");
			}else fpArray[im]=0;
		}
			...
			for(pos=0; pos<seqlen[iseq]-8; ++pos){
				...
				for(i=0; i<L->num; ++i){
					...
					for(j=0; j<M->len; ++j){
						...
						if(missing){ break; }
						//strongly disagree with PWM? when Pi <0.0111670898 in core

						if(j>=M->offset && j<M->offset+8) continue;
						score += M->m[j][pattern[j]];

						//early stop zzz added
						if(score+M->max_remain_gain_forward[j]<M->scoreThresh )
						{
							missing=1;
							break;
						}
					...
					if (USESTACK){
						for(j=nstack-1; j>=0; j-=3){
							...
							if(im1!=im && start-pos1 >= len1*0.5 && pos1+len1-start < len*0.5) continue;
							if(start-pos1>=len) break;
							if(im1!=im)
								continue;
					...
					} else if (position[im].num>1){ // just keep track of previous position of same matrix
						pos1=position[im].pos[position[im].num-1];
						score1=position[im].score[position[im].num-1];
						if (start-pos1<len){
							if (score1>=score){
								...
								if(!remove_position(&position[im],pos1,M->len)) error_message("Deletion Failed");
							}
						}
						if(redundant){ continue;} //zzz 
					}

					if(im<mmm||im>=mmm+1000)//maximum open 1000 files at the same time!
						continue;
					if (USESTACK){
						stack[nstack++] = im;
						stack[nstack++] = start;
			...
			for(im=0; im<nPattern; ++im){
				...
				for(i=0; i<P->num; ++i){
					if(fpArray[im]){  //print output
						...
						//fprintf(fpArray[im],"%d\t%d\t%s",iseq,P->pos[i]);
						fprintf(fpArray[im],"%d\t%d\t%.4f\t%s",iseq,P->pos[i],P->score[i],strands[(P->dir[i]+1)/2]);
						//printf("%s\t",buffer);
						//printf("%d\t%d\n",iseq,P->pos[i]);
						//fprintf(fpArray[im],"%d\t%d\t%f\t%s",iseq,P->pos[i],P->score[i],buffer);
						/*if(cons){
	...
	/***********************************************/
{
	...
	if(nargs < 4){ printf("patternScan -i motifFile -f fastaFile -o outputFile [-cons conservationFile, -maxlen maxSequenceLength, -strand strandOption, -n numberOfMotifs, -fp falsePositives, -thresh, matchThreshold, -userep, -one]"); exit(1); }
	check(p = (PARAM*)calloc(1,sizeof(PARAM)));
	p->max_length = 5000000000;
	p->N_matrix = 40000;
	while(iarg < nargs){
		...
		else if(!strcmp(argv[iarg],"-bg") && iarg < nargs-1) p->bgFile=copy_string(argv[++iarg]);
		else if(!strcmp(argv[iarg],"-cons") && iarg < nargs-1) p->conservFile=copy_string(argv[++iarg]);
		else if(!strcmp(argv[iarg],"-maxlen") && iarg < nargs-1) sscanf(argv[++iarg],"%d",&p->max_length);
	...
	/***********************************************/
{
	...
	if(p->falsePositives){
		...
		long lengthRand;
		if (p->bgFile){
			lengthRand = read_file(p->bgFile,p->use_repeats,seqRandom,seqlenRand,&nseqRand,seqNames);
			printf("load bg file: %d\n",nseqRand);
			nseqRand=500;
			//to use more random seq if p is smaller
			if (p->falsePositives<1) nseqRand/=p->falsePositives;
			lengthRand=nseqRand*1000;//assume length of each seq in bgfile is 1000
		} else {
			lengthRand = get_random_sequence(seqRandom,seqlenRand,&nseqRand,p->falsePositives);
		}
		make_lookup_table(lookupTable,Matrix,nPattern,complement,p->strand);
		find_motifs(seqRandom,seqlenRand,nseqRand,lookupTable,Matrix,nPattern,p,"/dev/null",seqNames,NULL,scores,nhits,1);
		...
		for(im=0; im<nPattern; ++im){
			...
			long N = nhits[im];
			//zzz add M->len
			long expected = ceil((double)(lengthRand)/10000*p->falsePositives);// considering overlapping filter -nPattern*8

			if(N >= expected || M->scoreThresh <= 0.301){
				float x = M->scoreThresh;
				if(N >= expected){
					sortem(N, scores[im], 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
					int upperindex;
					int lowerindex;
					for(lowerindex=N-expected;lowerindex>=0&&scores[im][lowerindex]==scores[im][N-expected];lowerindex--);
					lowerindex++;
					for(upperindex=N-expected;upperindex<N&&scores[im][upperindex]==scores[im][N-expected];upperindex++);
					upperindex--;
					printf("%s %d %d %d\n",M->name,expected,N-lowerindex,N-upperindex);
					if (1.0*(expected)/(N-lowerindex)>1.0*(N-upperindex)/(expected))
						M->scoreThresh = scores[im][N-expected]+0.000001; //no problem with zero occurrence
					else 
						M->scoreThresh = scores[im][N-expected];
					//M->scoreThresh = scores[im][N-expected]+0.000001; //no problem with zero occurrence
					//M->scoreThresh = scores[im][N-expected]+0.0000001;//avoid having zero occurrence
					//M->scoreThresh = scores[im][N-expected]*1.0000000001;//avoid having zero occurrence
					if (M->scoreThresh>scores[im][N-1])
						M->scoreThresh=scores[im][N-1]; 

					//printf("%s random hit: %d %f\n",M->name,M->scoreThresh,expected);
				}



				if(M->scoreThresh < 0.301) M->scoreThresh = 0.301;//  0.301 mean FDR=0.5, cant be worse
				M->scoreThreshCore += M->scoreThresh - x; //since the adjust for the non-core still the same
				//printf("Done: %d %.4f\n",im,M->scoreThresh);
				continue;
			}
			else
				printf("%s\tN:%d\tThresh:%f\n", M->name,N,M->scoreThresh);
		}
	...
	make_lookup_table(lookupTable,Matrix,nPattern,complement,p->strand);
	char fname1234[500];
	sprintf(fname1234,"%s/scanresult.txt",p->outputFile);
	fp = fopen(fname1234,"w");
	//_mkdir(p->outputFile);
	...
	fprintf(fp,"\n");
	find_motifs(sequence1,seqlen1,n1,lookupTable,Matrix,nPattern,p,p->outputFile,seqNames,conservation,NULL,nhits,0);
	long N = 0;
	for(im=0; im<nPattern; ++im){
		N += nhits[im];
	}

...
outf.close()
##########################prepare motifbed file##########################################
#using default threshold, without -findcutoff option
txtOutput=commands.getoutput('/home/zhizhuo/public_html/galaxy-dist/tools/mytools/BEDTools-Version-2.10.1/bin/calcMIscoreBed -a '+peakbed+' -b '+motifbed+'   -cotf')
(score,mu,sigma,thresh,noiselevel)=eval(txtOutput)
print(posfile+'\t'+str(float(score))+'\t'+str(int(sigma))+'\t'+str(float(thresh))+'\t'+str(int(mu))+'\t'+str(float(noiselevel)))

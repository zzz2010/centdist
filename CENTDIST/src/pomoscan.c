	...
	/***********************************************/
{
	...
	sprintf(matlistfilename,"%s/matlist.txt",p->outputFile);
...
/***********************************************/
void  find_motifsbg (int **sequence, long *seqlen, long nseq, LOOKUP *lookupTable,
...
/***********************************************/
{
	...
	char myfilename[500];
	for(mmm=0; mmm<nPattern; mmm=mmm+maxmm)
	{			

		for(im= mmm; im<mmm+maxmm&&im<nPattern; ++im)
		{
			...
			for(im=0; im<nPattern; ++im){
				...
				for(i=0; i<P->num; ++i){
					if(fpArray[im]){  //print output
						...
						//fprintf(fpArray[im],"%d\t%d\t%s",iseq,P->pos[i],buffer);
...
/***********************************************/
void  find_motifsfg (int **sequence, long *seqlen, long nseq, LOOKUP *lookupTable,
		MATRIX *Matrix, int nPattern, PARAM *p, char* outdir, char **seqNames, int **conservation, float **scores, long *nhits,int bgmode)
/***********************************************/
{
	...
	FILE ** fpArray;
	char* filename;
	check(matches = (MATCH*)calloc(nPattern,sizeof(MATCH)));
	check(buffer = (char*)malloc(300*sizeof(char)));
	check(stack = (float*)malloc(NSTACK*sizeof(float)));
	check(pattern = (int*)malloc(MAXLENGTH*sizeof(int)));
	check(position = (POSITION*)calloc(nPattern,sizeof(POSITION)));
	...
	{			
		...
		for(im= mmm; im<mmm+1&&im<nPattern; ++im)
		{
			sprintf(myfilename,"%s/%s.pos",outdir,Matrix[im].name);
			if (bgmode==0){
				fpArray[im]=fopen(myfilename,"w+");
		...
		for(iseq=0; iseq<nseq; ++iseq){
			...
			for(pos=0; pos<seqlen[iseq]-8; ++pos){
				...
				for(i=0; i<L->num; ++i){
					...
					if(!matches->alloc){
						matches->alloc=2*nseq;
						check(matches->data=(MATCH*)malloc(matches->alloc*sizeof(MATCH)));
					}else if(matchesN==matches->alloc){
						matches->alloc*=2;
						check(matches->data=(MATCH*)realloc(matches->data,matches->alloc*sizeof(MATCH)));
					}
	...
	/***********************************************/
{
	...
	if(p->falsePositives){
		...
		find_motifsbg(seqRandom,seqlenRand,nseqRand,lookupTable,Matrix,nPattern,p,"/dev/null",seqNames,NULL,scores,nhits,1);
	...
	fprintf(fp,"\n");
	find_motifsfg(sequence1,seqlen1,n1,lookupTable,Matrix,nPattern,p,p->outputFile,seqNames,conservation,NULL,nhits,0);
	long N = 0;
	for(im=0; im<nPattern; ++im){
		N += nhits[im];
	}

	...
	for(im=0; im<nPattern; ++im){
		...
		for(i=0; i<P->num; ++i){
			if(fpArray[im]){  //print output
				...
				fprintf(fpArray[im],"%d\t%d\t%.4f\t%s\t%s",iseq,P->pos[i],P->score[i],strands[(P->dir[i]+1)/2],buffer);
				//printf("%s\t",buffer);
				//printf("%d\t%d\n",iseq,P->pos[i]);
...
if(p->falsePositives){
	...
	for(im=0; im<nPattern; ++im){
		M = &Matrix[im];
		long N = nhits[im];
		long expected = (double)lengthRand*nseqRand/totalnseqRand/10000*p->falsePositives;
		if(N >= expected || M->scoreThresh <= -999999){
			float x = M->scoreThresh;
			if(N >= expected){
				sortem(N, scores[im], 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
				M->scoreThresh = scores[im][N-expected];
			}
			if(M->scoreThresh < -999999) M->scoreThresh = -999999;
			//M->scoreThreshCore += M->scoreThresh - x;
			//printf("Done: %d %.4f\n",im,M->scoreThresh);
			M->scoreThreshCore =-999999;
			//continue;
		}
		printf("%d %d %f %f\n",N,expected,M->scoreThresh,M->scoreThreshCore);	
	}

...
for doonce in [1]:
	...
	start=time()
	s=("%s/pomoscan -i '%s' -o '%s' -f '%s' -fp '1' -bg %s"%(params['progdir'],params['matfile'],params['outputdir'],fastafile,randbgfas))
	print(s)
	if (os.system(s)):
		print("ERROR OCCURRED")
		break
	...
	start=time()
	s=("R %s --no-save --args --slave < %s/plotgraphs.R"%(params['outputdir'],params['progdir']))
	print(s)
	if (os.system(s)):
		print("ERROR OCCURRED")
		break
	s="find %s -name '*.jpg' -exec convert {} -thumbnail x50 {}.tn.jpg ';'"%params['outputdir']
	print(s)
	if (os.system(s)):
		print("ERROR OCCURRED")
		break
